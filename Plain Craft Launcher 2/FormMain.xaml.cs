...
public partial class FormMain
{
    ...
    // 小teto定制：Win32 API 用于真正的全屏（覆盖整个屏幕包括任务栏）
    ...
    // 使用 Win32 API 设置真正的全屏（实际像素，不受 DPI 影响）
    private void SetFullscreenWin32(bool fullscreen)
    {
        try
        {
            ...
            if (fullscreen)
            {
                ...
                if (GetMonitorInfo(hMonitor, ref mi))
                {
                    ...
                    ModBase.Log($"[System] Win32全屏: 工作区实际像素 {w}x{h} (不遮挡任务栏)");
                    // 使用工作区实际像素设置窗口位置和大小（不遮挡任务栏）
            ...
            {
                // 退出全屏：取消置顶
                SetWindowPos(hwnd, (IntPtr)HWND_NOTOPMOST,
                    0, 0, 0, 0, 0x0001 | 0x0002); // SWP_NOMOVE | SWP_NOSIZE
            }
        }
        catch (Exception ex)
        {
            ModBase.Log(ex, "Win32全屏设置异常");
        }
    }
    private WindowStyle savedWindowStyle;
    private WindowState savedWindowState;
    private double savedWidth, savedHeight, savedLeft, savedTop;
    private CornerRadius savedCornerRadius;
    ...
    {
        // 小teto定制：检测启动时窗口是否为全屏大小（上次全屏操作残留），如果是则恢复默认大小
        try
        {
            var workArea = SystemParameters.WorkArea;
            if (Width >= workArea.Width - 10 && Height >= workArea.Height - 10)
            {
                ModBase.Log("[System] 检测到启动时窗口为全屏大小残留，恢复默认大小");
                Width = 1244;
                Height = 728;
                Left = (workArea.Width - 1244) / 2 + workArea.Left;
                Top = (workArea.Height - 728) / 2 + workArea.Top;
            }
        }
        catch { }
    ...
    // 小teto定制：全屏切换
    private void BtnTitleFullscreen_Click(object sender, EventArgs e)
    {
        ToggleFullscreen();
    }

    // 小teto定制：窗口大小/位置平滑动画（使用 CompositionTarget.Rendering，与渲染帧同步，60fps）
    private void AnimateWindowTo(double targetLeft, double targetTop, double targetWidth, double targetHeight, int durationMs = 180)
    {
        var startLeft = Left;
        var startTop = Top;
        ...
        // 使用 CompositionTarget.Rendering 代替 DispatcherTimer
        // 与 WPF 渲染帧同步，精度更高，更流畅
        ...
        renderHandler = (s, e) =>
        {
            ...
            // Quadratic ease in-out（比 Cubic 计算更快，更流畅）
            double eased;
            if (progress < 0.5)
    ...
    private void ToggleFullscreen()
    {
        try
        {
            if (!isFullscreen)
            {
                ...
                ModBase.Log("[System] 开始进入全屏...");

                // 处理 WindowChrome 圆角
                var chrome = System.Windows.Shell.WindowChrome.GetWindowChrome(this);
                if (chrome != null)
                {
                    savedCornerRadius = chrome.CornerRadius;
                    chrome.CornerRadius = new CornerRadius(0);
                    ModBase.Log("[System] WindowChrome 圆角已设置为 0");
                }
                ...
                // 设置 WindowStyle=None（先切换样式，再动画）
                WindowStyle = WindowStyle.None;

                // 关键：不使用 WindowState=Maximized（WPF bug）
                // 直接使用 Win32 API 设置精确全屏（不遮挡任务栏），无过渡动画
                WindowStyle = WindowStyle.None;
                SetFullscreenWin32(true);
                Topmost = true;
                isFullscreen = true;
                ModBase.Log("[System] 已进入全屏（无动画）");

                BtnTitleFullscreen.SvgIcon = "lucide/minimize-2";
                ModBase.Log("[System] 进入全屏模式成功, WindowStyle=" + WindowStyle + ", Size=" + Width + "x" + Height);
            }
            else
            {
                // 退出全屏
                ModBase.Log("[System] 开始退出全屏...");

                var chrome = System.Windows.Shell.WindowChrome.GetWindowChrome(this);
                if (chrome != null)
                {
                    chrome.CornerRadius = savedCornerRadius;
                    ModBase.Log("[System] WindowChrome 圆角已恢复");
                }

                Topmost = false;
                // 先取消 Win32 全屏（恢复 WPF 窗口控制）
                SetFullscreenWin32(false);
                ModBase.Log($"[System] 退出全屏: 保存的大小 {savedWidth}x{savedHeight}, 当前大小 {Width}x{Height}");

                // 关键：先强制设置 WPF 窗口大小为保存的值（Win32 可能覆盖了 WPF 的 Height）
                // 不使用动画，直接设置确保高度正确恢复
                ...
                UpdateLayout();
                ModBase.Log($"[System] 退出全屏完成: 恢复后大小 {Width}x{Height}");
                isFullscreen = false;

                BtnTitleFullscreen.SvgIcon = "lucide/maximize";
                ModBase.Log("[System] 退出全屏模式成功");
            }
        }
        catch (Exception ex)
        {
            ModBase.Log(ex, "全屏切换异常");
    ...
    public enum PageSubType
    {
        ...
        DownloadBedrock = 20,
        DownloadOtherGames = 21,
        DownloadLegends = 22,
        DownloadHome = 23,
        DownloadBedrockResources = 24,
    ...
    private void PageChangeActual(PageStackData stack, PageSubType subType)
    {
        ...
        try
        {
            ...
            switch (stack.page)
            {
                ...
                case PageType.Tools: // 工具（只显示百宝箱等工具，无联机）
                    {
                        ModMain.frmToolsLeft ??= new PageToolsLeft();
                        ModMain.frmToolsLeft.SetLobbyMode(false);
                ...
                case PageType.Lobby: // 联机（顶部导航独立入口，只显示联机大厅，无百宝箱）
                    {
                        ModMain.frmToolsLeft ??= new PageToolsLeft();
                        ModMain.frmToolsLeft.SetLobbyMode(true);
                        subType = ModMain.frmToolsLeft.pageID;
                        PageChangeAnim(ModMain.frmToolsLeft, (FrameworkElement)ModMain.frmToolsLeft.PageGet(subType));
                        break;
                    }
    ...
    private void BtnExtraShutdown_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            ...
            foreach (var Watcher in ModWatcher.mcWatcherList)
                Watcher.Kill();
            // 小teto定制：同时结束所有基岩版（BE）进程（标记为用户主动结束，不弹错误报告）
            ModOtherGames.BedrockUserKilled = true;
            ModOtherGames.CurrentBedrockProcess = null;
            foreach (var p in Process.GetProcessesByName("Minecraft.Windows"))
            {
                try { p.Kill(); } catch { }
            }
            // 小teto定制：点击后立即隐藏「结束进程」按钮（用户已主动结束，不等异步 WaitForExit 线程）
            ModWatcher.hasRunningMinecraft = false;
            BtnExtraShutdown.ShowRefresh();
