...
public partial class PageToolsGameLink
{
    ...
    private void PageLinkLobby_OnPageEnter()
    {
        if (_beMode)
        {
            PanEula.Visibility = Visibility.Collapsed;
            PanSelect.Visibility = Visibility.Collapsed;
            PanFinish.Visibility = Visibility.Collapsed;
            PanGravityCone.Visibility = Visibility.Visible;
            // 小teto定制：检测当前目录是否已有 GravityCone CLI，避免每次都提示下载
            if (File.Exists(GravityConeCliPath))
            {
                LabGcCliStatus.Text = "CLI 已就绪，可点击启动";
                BtnGcDownload.IsEnabled = true;
            }
            else
            {
                LabGcCliStatus.Text = "CLI 不存在，请先下载";
                BtnGcDownload.IsEnabled = true;
            }
            return;
        }
        ...
        PanGravityCone.Visibility = Visibility.Collapsed;
    }

    #endregion

    #region 小teto定制：BE 联机（GravityCone / PaperConnect）

    private readonly GravityConeClient _gc = new();
    private bool _beMode;
    private string GravityConeCliPath =>
        Path.Combine(ModBase.exePath, "GravityCone", "gravitycone-cli-windows-amd64.exe");

    private void BtnJeLink_Click(object sender, MouseButtonEventArgs e)
    {
        BtnJeLink.ColorType = MyButton.ColorState.Highlight;
        BtnBeLink.ColorType = MyButton.ColorState.Normal;
        _beMode = false;
        PageLinkLobby_OnPageEnter();
    }

    private void BtnBeLink_Click(object sender, MouseButtonEventArgs e)
    {
        BtnBeLink.ColorType = MyButton.ColorState.Highlight;
        BtnJeLink.ColorType = MyButton.ColorState.Normal;
        _beMode = true;
        PageLinkLobby_OnPageEnter();
    }

    private async void BtnGcDownload_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            BtnGcDownload.IsEnabled = false;
            LabGcCliStatus.Text = "正在查询最新版本…";
            using var client = new HttpClient();
            client.Timeout = TimeSpan.FromMinutes(10);
            client.DefaultRequestHeaders.UserAgent.ParseAdd("PCL2-CE");
            var jsonStr = await client.GetStringAsync(
                "https://api.github.com/repos/Tianpao/GravityCone/releases/latest");
            var json = JsonNode.Parse(jsonStr)!.AsObject();
            string? url = null;
            foreach (var a in json["assets"]!.AsArray())
            {
                var name = a!["name"]?.ToString() ?? "";
                if (name.Contains("windows-amd64") &&
            ...
            {
                LabGcCliStatus.Text = "未找到 Windows 版 CLI";
                return;
            }
            LabGcCliStatus.Text = "正在下载 GravityCone CLI…";
            Directory.CreateDirectory(Path.GetDirectoryName(GravityConeCliPath)!);
            var bytes = await client.GetByteArrayAsync(url);
            await File.WriteAllBytesAsync(GravityConeCliPath, bytes);
            LabGcCliStatus.Text = "CLI 已就绪，可点击启动";
        }
        catch (Exception ex)
        {
            LabGcCliStatus.Text = "下载失败：" + ex.Message;
        }
        finally
        {
            BtnGcDownload.IsEnabled = true;
    ...
    private async void BtnGcStart_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            if (!File.Exists(GravityConeCliPath))
            ...
            {
                _gc.Dispose();
                LabGcCliStatus.Text = "CLI 已停止";
                return;
            }
            LabGcCliStatus.Text = "正在启动 CLI…";
            await _gc.StartAsync(GravityConeCliPath);
            LabGcCliStatus.Text = "CLI 运行中，可创建/加入房间";
        }
        catch (Exception ex)
        {
            LabGcCliStatus.Text = "启动失败：" + ex.Message;
        }
    }

    private async void BtnGcCreate_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            var name = TextGcPlayerName.Text?.Trim();
            if (string.IsNullOrEmpty(name))
            {
                HintService.Hint("请输入玩家名", HintType.Error);
                return;
            }
            if (!_gc.IsRunning)
            {
                HintService.Hint("请先启动 CLI", HintType.Error);
                return;
            }
            var r = await _gc.CreatePaperConnectRoomAsync(name);
            LabGcRoomInfo.Text = "房间创建成功\n联机码：" + r.Code +
                                 "\n游戏端口：" + r.GamePort +
                                 "\n（复制联机码发给好友加入）";
            LabGcStatus.Text = "已创建房间，等待好友加入…";
        }
        catch (Exception ex)
        {
            LabGcRoomInfo.Text = "创建失败：" + ex.Message;
        }
    }

    private async void BtnGcJoin_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            var code = TextGcJoinCode.Text?.Trim();
            var name = TextGcPlayerName.Text?.Trim();
            if (string.IsNullOrEmpty(code))
            {
                HintService.Hint("请输入联机码", HintType.Error);
                return;
            }
            if (string.IsNullOrEmpty(name))
            {
            ...
            var r = await _gc.JoinRoomAsync(code, name);
            LabGcJoinInfo.Text = "已加入房间\n房间：" + r.RoomCode +
                                 "\n主机地址：" + r.HostAddress + ":" + r.GamePort +
                                 "\n在线：" + r.OnlineCount;
            LabGcStatus.Text = "已加入房间，正在连接…";
        }
        catch (Exception ex)
        {
            LabGcJoinInfo.Text = "加入失败：" + ex.Message;
        }
    }

    private async void BtnGcLeave_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            if (_gc.IsRunning)
            {
                await _gc.LeaveRoomAsync();
                LabGcStatus.Text = "已离开房间";
                LabGcRoomInfo.Text = "";
                LabGcJoinInfo.Text = "";
            }
        }
        ...
        {
            LabGcStatus.Text = "离开失败：" + ex.Message;
        }
    }
