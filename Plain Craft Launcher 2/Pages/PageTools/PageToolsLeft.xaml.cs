...
public partial class PageToolsLeft
{
    ...
    private bool isLobbyMode; // 当前显示模式：true=联机（只显示大厅），false=工具（只显示百宝箱）
    ...
    private void PageLinkLeft_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        // 选择第一个未被禁用的子页面
        // 小teto定制：加载时确保按当前模式（联机/工具）显示正确侧边栏项
        ApplyModeVisibility();
        if (isPageSwitched) 
            return;
        var hideCfg = Config.Preference.Hide;
        // 按当前显示模式选择默认项：联机模式默认联机大厅，工具模式默认百宝箱
        var lobbyVisible = ItemGameLink.Visibility == Visibility.Visible;
        var toolVisible = ItemTest.Visibility == Visibility.Visible;
        if (lobbyVisible && !hideCfg.ToolsGameLink)
            ItemGameLink.SetChecked(true, false, false);
        else if (toolVisible && !hideCfg.ToolsTest)
            ItemTest.SetChecked(true, false, false);
        else if (lobbyVisible)
            ItemGameLink.SetChecked(true, false, false);
        else
            ItemTest.SetChecked(true, false, false);
        // 同步 pageID 与当前模式（防止 Loaded 时默认 Checked 触发 PageChange 走到错误目标）
        pageID = isLobbyMode ? FormMain.PageSubType.ToolsGameLink : FormMain.PageSubType.ToolsTest;
    }

    /// <summary>小teto定制：确保加载后仍按当前模式显示（联机模式绝不显示百宝箱/工具）。</summary>
    private void ApplyModeOnLoaded()
    {
        ApplyModeVisibility();
    }

    /// <summary>
    ///     设置工具页显示模式：true=顶部联机入口（只显示联机大厅），false=工具入口（只显示百宝箱等工具）。
    ///     右侧页面的实际切换由 FormMain 依据 pageID 统一完成，避免此处 PageChange 与主页面切换动画竞争。
    /// </summary>
    private void ApplyModeVisibility()
    {
        TextGameLinkCategory.Visibility = isLobbyMode ? Visibility.Visible : Visibility.Collapsed;
        ItemGameLink.Visibility = isLobbyMode ? Visibility.Visible : Visibility.Collapsed;
        TextToolsCategory.Visibility = isLobbyMode ? Visibility.Collapsed : Visibility.Visible;
        ItemTest.Visibility = isLobbyMode ? Visibility.Collapsed : Visibility.Visible;
    }
    ...
    public void SetLobbyMode(bool isLobby)
    {
        isLobbyMode = isLobby;
        ApplyModeVisibility();
        // 同步当前模式对应的页面 ID，并勾选正确项（此时 pageID 与目标一致，SetChecked 触发的 PageChange 会直接返回，不干扰）
        pageID = isLobby ? FormMain.PageSubType.ToolsGameLink : FormMain.PageSubType.ToolsTest;
        if (isLobby)
            ItemGameLink.SetChecked(true, false, false);
        else
            ItemTest.SetChecked(true, false, false);
