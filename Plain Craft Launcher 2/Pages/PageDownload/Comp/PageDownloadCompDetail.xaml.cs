...
public partial class PageDownloadCompDetail
{
    ...
    public void Save_Click(object sender, EventArgs e)
    {
        ...
        ModBase.RunInNewThread(() =>
        {
            try
            {
                // 小teto定制：BE 资源 → 自动下载并解压安装到当前选中的 BE 版本（不弹保存位置）
                if ((_project?.Website ?? "").Contains("minecraft-bedrock"))
                {
                    BedrockResourceSaveToSelected(file);
                    return;
                }
    ...
    /// <summary>小teto定制：BE 资源下载 → 自动解压安装到当前选中的 BE 版本（不弹保存位置对话框）。</summary>
    private void BedrockResourceSaveToSelected(ModComp.CompFile file)
    {
        try
        {
            // 1. 确定目标 BE 版本文件夹（多级回退：详情页筛选版本 -> 主界面选中版本 -> 本地已有 BE 版本）
            var versionName = ModOtherGames.SelectedBedrockVersion;
            if (!string.IsNullOrEmpty(_targetInstance) &&
                _targetInstance != Lang.Text("Download.Comp.Filter.Version.Any") &&
                _targetInstance != Lang.Text("Download.Comp.Filter.Version.AllInputAvailable"))
                versionName = _targetInstance;
            string verDir = null;
            foreach (var cand in new[] { versionName, ModOtherGames.SelectedBedrockVersion })
            {
                if (string.IsNullOrEmpty(cand)) continue;
                var c = Path.Combine(ModFolder.mcFolderSelected, "bedrock_versions", cand);
                if (Directory.Exists(c)) { verDir = c; versionName = cand; break; }
            }
            if (verDir is null)
            {
                // 最后尝试本地已有 BE 版本
                var locals = ModOtherGames.GetBedrockLocalVersions();
                if (locals.Count > 0)
                {
                    versionName = locals[0];
                    verDir = Path.Combine(ModFolder.mcFolderSelected, "bedrock_versions", versionName);
                }
            }
            if (verDir is null)
            {
                ModBase.RunInUi(() => ModMain.MyMsgBox(
                    "未检测到可用的基岩版版本文件夹。请先在「启动」页选择一个 BE 版本，或确认当前文件夹的 bedrock_versions 目录不为空。",
                    "请先选择基岩版版本", "知道了", isWarn: true, forceWait: true));
                return;
            }

            // 2. 临时下载文件（保留原始扩展名）
            var ext = Path.GetExtension(file.FileName);
            if (string.IsNullOrEmpty(ext)) ext = ".mcpack";
            var tempFile = Path.Combine(Path.GetTempPath(), "PCL_BE_" + Guid.NewGuid().ToString("N") + ext.ToLower());
            ...
            var loaders = new List<ModLoader.LoaderBase>
            {
                new LoaderDownload(Lang.Text("Download.Comp.Detail.DownloadFile"),
                    new List<DownloadFile>
                    {
                        file.ToNetFile(tempFile, ModComp.DownloadReason.Standalone, null)
                    })
                {
                    ProgressWeight = 6,
                    block = true
            ...
            // 4. 等待下载完成 → 解压安装 → 清理临时文件
            ModBase.RunInNewThread(() =>
            {
                try
                {
                    while (loader.State == ModBase.LoadState.Waiting || loader.State == ModBase.LoadState.Loading)
                        System.Threading.Thread.Sleep(200);
                    if (loader.State == ModBase.LoadState.Finished && File.Exists(tempFile))
                    {
                        var result = ModOtherGames.InstallBedrockResource(verDir, tempFile);
                        ModBase.Log("[Bedrock] BE 资源安装结果：" + result);
                        ModBase.RunInUi(() =>
                        {
                            if (result.Contains("成功"))
                                HintService.Hint("已安装到 " + versionName + "：" + Path.GetFileName(file.FileName));
                            else
                                HintService.Hint("安装失败：" + result);
                        });
                    }
                }
                catch (Exception ex)
            ...
            }, "Bedrock Resource Install");
        }
        catch (Exception ex)
        {
            ModBase.Log(ex, "保存 BE 资源失败", ModBase.LogLevel.Feedback,
                userSummary: Lang.Text("Download.Comp.Error.OperationFailed"));
        }
    }
    ...
    private string GetGroupedVersionName(string name, bool groupedByDrop, bool foldOld)
    {
        ...
        if (groupedByDrop)
        {
            // 小teto定制：BE 年号版（20~26）按大版本分组，避免 26.45 -> 30.5 之类的 drop 映射脏名
            var beMajor = name.Split('.')[0];
            if (int.TryParse(beMajor, out var beM) && beM >= 20 && beM <= 30)
                return beMajor;
