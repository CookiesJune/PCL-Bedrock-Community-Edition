...
public partial class PageDownloadInstall
{
    ...
    public PageDownloadInstall()
    {
        ...
        // 小teto定制：返回按钮已改为提示文字（该按钮修不好BUG，请自行刷新页面返回）
    ...
    {
        ModBase.Log("[DownloadInstall] EnterSelectPage called, isInSelectPage=" + isInSelectPage);
        if (isInSelectPage)
            return;
        isInSelectPage = true;
    ...
    {
        ModBase.Log("[DownloadInstall] ExitSelectPage called, isInSelectPage=" + isInSelectPage);
        if (!isInSelectPage)
            return;
