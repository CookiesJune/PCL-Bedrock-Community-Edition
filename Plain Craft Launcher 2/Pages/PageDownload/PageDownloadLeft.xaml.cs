...
public partial class PageDownloadLeft : IRefreshable
{
    ...
    public void Refresh(FormMain.PageSubType subType)
    {
        switch (subType)
        {
            case FormMain.PageSubType.DownloadHome:
            {
    ...
    private void ItemInstall_Click(object sender, MouseButtonEventArgs e)
    {
        if (!ItemInstall.Checked)
            return;
        PageChange(FormMain.PageSubType.DownloadHome);
    }

    /// <summary>
    ///     由下载中心横幅调用：切换页面并让侧边栏高亮跟随。
    ///     suppressCheck 用于防止设置 Checked 时触发 Check→PageCheck 再次切换页面（曾导致切到 Java 页后立刻弹回下载中心）。
    /// </summary>
    private bool suppressCheck;

    public void GoTo(FormMain.PageSubType subType)
    {
        suppressCheck = true;
        try
        {
    ...
    ///     当前页面的编号。
    /// </summary>
    public FormMain.PageSubType pageID = FormMain.PageSubType.DownloadHome;
    ...
    public PageDownloadLeft()
    {
        ...
        ItemBedrockResources.Check += PageCheck;
    ...
    public object PageGet(FormMain.PageSubType id)
    {
        ...
        switch (id)
        {
            case FormMain.PageSubType.DownloadHome:
            {
                if (ModMain.frmDownloadHome is null)
                    ModMain.frmDownloadHome = new PageDownloadHome();
                return ModMain.frmDownloadHome;
            }
            case FormMain.PageSubType.DownloadInstall:
            {
            ...
            case FormMain.PageSubType.DownloadBedrockResources:
            {
                if (ModMain.frmDownloadBedrockResources is null)
                    ModMain.frmDownloadBedrockResources = new PageDownloadBedrockResources();
                return ModMain.frmDownloadBedrockResources;
            }
            case FormMain.PageSubType.DownloadClient:
            {
            ...
            case FormMain.PageSubType.DownloadBedrock:
            {
                if (ModMain.frmDownloadBedrock is null)
                    ModMain.frmDownloadBedrock = new PageDownloadBedrock();
                return ModMain.frmDownloadBedrock;
            }
            case FormMain.PageSubType.DownloadOtherGames:
            case FormMain.PageSubType.DownloadLegends:
            {
                if (ModMain.frmDownloadOtherGames is null)
                    ModMain.frmDownloadOtherGames = new PageDownloadOtherGames();
                return ModMain.frmDownloadOtherGames;
            }
