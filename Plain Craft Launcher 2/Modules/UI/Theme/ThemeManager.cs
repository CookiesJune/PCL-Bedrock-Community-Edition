...
public static class ThemeManager
{
    ...
    // 主页面背景
        // 主页面背景 (custom build: light green / dark green)
    private static void RefreshBackground()
    {
        var (hue, _, _) = ThemeService.GetCurrentThemeArgs();
        ...
        brush.GradientStops.Add(new GradientStop
            { Offset = -0.1d, Color = LabColor.FromLch(0.82d * darkLight, 0.05, hue - 10) });
        brush.GradientStops.Add(new GradientStop
            { Offset = 0.4d, Color = LabColor.FromLch(0.94d * darkLight, 0.07, hue) });
        brush.GradientStops.Add(new GradientStop
            { Offset = 1.1d, Color = LabColor.FromLch(0.82d * darkLight, 0.05, hue + 10) });
