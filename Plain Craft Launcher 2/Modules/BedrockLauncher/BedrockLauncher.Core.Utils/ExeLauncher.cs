...
public class ExeLauncher
{
	...
	private static extern bool CreateProcessWithTokenW(nint hToken, LogonFlags dwLogonFlags, string lpApplicationName, string lpCommandLine, CreationFlags dwCreationFlags, nint lpEnvironment, string lpCurrentDirectory, ref STARTUPINFO lpStartupInfo, out PROCESS_INFORMATION lpProcessInformation);
	...
	public static Process LaunchWithLowPrivilege(string exePath, string arguments = "")
	{
		...
		try
		{
			...
			if (!CreateProcessWithTokenW(phNewToken, LogonFlags.LOGON_WITH_PROFILE, null, lpCommandLine, (CreationFlags)1056, IntPtr.Zero, directoryName, ref lpStartupInfo, out lpProcessInformation))
			{
