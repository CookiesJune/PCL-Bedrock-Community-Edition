...
public class ModSetup
{
    ...
    public static void LaunchFolderSelect(string value)
    {
        ...
        // 小teto定制：切换文件夹后从 PCL.ini 恢复 BE 版本选择（修复重启后版本选择丢失的BUG）
        try
        {
            var beVerDir = Path.Combine(ModFolder.mcFolderSelected, "bedrock_versions");
            if (Directory.Exists(beVerDir))
            {
                // 小teto定制：优先读 BedrockVersion 键（独立键，不与 Java 的 Version 键冲突），回退 Version 键
                ...
                if (!string.IsNullOrEmpty(savedVer) && Directory.Exists(Path.Combine(beVerDir, savedVer)))
                {
                    ModOtherGames.SelectedBedrockVersion = savedVer;
                    ModBase.Log("[Setup] 恢复 BE 版本选择：" + savedVer);
                }
                else
                {
