    ...
    public class LocalCompFile
    {
        ...
        public static bool IsModFile(string path)
        {
            if (path is null)
                return false;
            path = path.ToLower();
            // 小teto定制：BE 行为包文件夹（__FOLDER__ 结尾）也视为 Mod 文件
            if (path.EndsWithF("__folder__", true))
                return true;
            if (!path.Contains("."))
                return false;
            if (path.EndsWithF(".jar", true) || path.EndsWithF(".zip", true) || path.EndsWithF(".litemod", true) ||
                ...
                path.EndsWithF(".mcaddon", true)) // 小teto定制：BE 行为包
                return true;
            return false;
        }
        ...
        public static bool IsCompFile(string path, CompType compType)
        {
            ...
            switch (compType)
            {
                ...
                {
                    // 小teto定制：BE 行为包文件夹
                    if (path.EndsWithF("__folder__", true)) return true;
                    return IsModFile(path);
                }
                case CompType.ResourcePack:
                case CompType.Shader:
                {
                    // 小teto定制：BE 资源包文件夹或 .mcpack 文件
                    if (path.EndsWithF("__folder__", true)) return true;
                    return path.EndsWithF(".zip", true) || path.EndsWithF(".mcpack", true);
                }
        ...
        public string GetLogo()
        {
            ...
            // 小teto定制：BE 行为包/资源包文件夹优先显示自己的图标
            if (IsFolder)
            {
                try
                {
                    var folderPath = ActualPath;
                    // BE 包常见的图标文件名
                    string[] beIconNames = { "pack_icon.png", "icon.png", "pack_icon.jpg", "icon.jpg", "pack_icon.jpeg" };
                    foreach (var iconName in beIconNames)
                    {
                        var iconPath = Path.Combine(folderPath, iconName);
                        if (File.Exists(iconPath))
                            return iconPath;
                    }
                    // 检查 manifest.json 中是否指定了图标
                    var manifestPath = Path.Combine(folderPath, "manifest.json");
                    if (File.Exists(manifestPath))
                    {
                        try
                        {
                            var manifest = ModBase.ReadFile(manifestPath);
                            if (!string.IsNullOrEmpty(manifest))
                            {
                                // 简单匹配 "icon": "xxx.png"
                                var match = System.Text.RegularExpressions.Regex.Match(manifest, @"""icon""\s*:\s*""([^""]+)""");
                                if (match.Success)
                                {
                                    var iconFromManifest = Path.Combine(folderPath, match.Groups[1].Value);
                                    if (File.Exists(iconFromManifest))
                                        return iconFromManifest;
                                }
                            }
                        }
                        catch { }
    ...
    private static void CompResourceListLoad(LoaderTask<CompLocalLoaderData, List<LocalCompFile>> loader)
    {
        try
        {
            ...
            if (Directory.Exists(loader.input.compPath))
            {
                ...
                {
                    // 小teto定制：检测是否为 BE 行为包/资源包目录（以文件夹形式存在的包）
                    // compPath 末尾可能有反斜杠，需要 TrimEnd 后再匹配
                    var bePackPath = rawName.Replace('/', '\\').TrimEnd('\\');
                    bool isBePackFolder = bePackPath.EndsWithF("\\behavior_packs", true) ||
                                          bePackPath.EndsWithF("\\resource_packs", true) ||
                                          bePackPath.EndsWithF("\\skin_packs", true);
                    try
                    {
                        if (isBePackFolder)
                        {
                            // BE：枚举一级子文件夹（每个包含 manifest.json 的文件夹是一个包）
                            var beDirInfo = new DirectoryInfo(loader.input.compPath);
                            foreach (var subDir in beDirInfo.EnumerateDirectories())
                            {
                                try
                                {
                                    // 只把包含 manifest.json 的文件夹识别为 BE 包
                                    if (File.Exists(Path.Combine(subDir.FullName, "manifest.json")))
                                        modList.Add(new LocalCompFile(Path.Combine(subDir.FullName, "__FOLDER__")));
                                }
                                catch (Exception ex)
                                {
                                    ModBase.Log(ex, $"处理 BE 文件夹失败：{subDir.FullName}");
                                }
                            }
                        }
                        foreach (var File in ModBase.EnumerateFiles(loader.input.compPath))
                            try
                            {
                                ...
                                {
                                    // BE 包目录：允许一级子目录下的文件（.mcaddon/.mcpack 可能在子目录）
                                    if (!isBePackFolder)
                                    {
