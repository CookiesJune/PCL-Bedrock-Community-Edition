...
public static class ModFolder
{
    ...
    private static void McFolderListLoadSub()
    {
        try
        {
            ...
            // 小teto定制：只保留最早添加的一个 BE 文件夹（其余从列表中移除，避免多个 BE 文件夹导致列表显示不稳定）
            try
            {
                var beList = cacheMcFolderList.Where(f => ModOtherGames.IsBedrockFolder(f.Location)).ToList();
                if (beList.Count > 1)
                {
                    var keep = beList[0]; // 列表顺序即添加顺序，第一个为最早添加
                    foreach (var extra in beList.Skip(1))
                    {
                        cacheMcFolderList.Remove(extra);
                        // 若当前选中的正是被移除的 BE 文件夹，切换到保留项
                        if (!string.IsNullOrEmpty(mcFolderSelected) &&
                            string.Equals(mcFolderSelected.TrimEnd('\\'), extra.Location.TrimEnd('\\'),
                                StringComparison.OrdinalIgnoreCase))
                        {
                            mcFolderSelected = keep.Location;
                            States.Game.SelectedFolder = keep.Location.Replace(ModBase.exePath, "$");
                        }
                    }
                    ModBase.Log("[小teto] 仅保留最早添加的 BE 文件夹：" + keep.Location +
                                "，移除多余 BE 文件夹 " + (beList.Count - 1) + " 个");
                }
            }
            catch (Exception ex)
            {
                ModBase.Log(ex, "[小teto] 清理多余 BE 文件夹失败");
            }
