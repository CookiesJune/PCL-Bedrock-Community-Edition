...
public sealed partial class ThemeService
{
    ...
    private static void _Start()
    {
        ...
        _LogStatus();
        // Custom build: default theme is TetoGreen (ink-green) for both light & dark; user can change it
        if (!Enum.IsDefined(typeof(ColorTheme), CurrentTheme))
            CurrentTheme = ColorTheme.TetoGreen;
        _RefreshAll();
    }
    ...
    {
        // custom build: no April Fools' theme tricks
    ...
    public static (int Hue, double LightAdjust, double ChromaAdjust) GetCurrentThemeArgs()
    {
        ...
        // 小teto定制：喜庆红改深红色；松木棕按深浅模式区分（深色深棕/浅色浅棕）
        if (theme == ColorTheme.PineBrown)
            return IsDarkMode ? (32, -0.08, -0.22) : (32, -0.04, 0.08);
        return theme switch
        {
            ColorTheme.SkyBlue => (235, 0.36, 0.2),
            ColorTheme.TetoGreen => (140, -0.05, -0.15),
            ColorTheme.DreamPurple => (280, 0, -0.1),
            ColorTheme.FestiveRed => (0, 0.08, -0.18), // 深红色（降低亮度）
            ColorTheme.VitalityOrange => (35, 0.1, 0.25),
            _ => (140, -0.05, -0.15)
        };
    }
